...
import {summEN} from  "../services/translation"
const NoteFormContext = createContext({});

export const NoteFormProviderSummEN = ({ children }) => {
  const [note, setNote] = useState({
    title: "wait the summarization will appear here ",
    details: "",
    category: "money",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (note.title && note.details) {
      setSubmitting(true);
      const result = await summEN(note);
      if (result) {
        setSubmitting(false);
        setNote((prevState) => {
          return {
